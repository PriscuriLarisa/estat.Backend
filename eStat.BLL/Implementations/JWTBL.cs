...
{
    public class JWTBL : BusinessObject, IJWT
    {
        private readonly string stringKey = "very secure key :)";

        public JWTBL(IDALContext dalContext) : base(dalContext)
        {
        }

        public string Generate(Guid id)
        {
            var symmetricSecurityKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(stringKey));
